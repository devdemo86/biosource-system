    ...
    if(isset($_SESSION['type_id'])) {
        ...
        if(isset($_POST['product'])) {
            ...
            if($queryexist->num_rows == 0) {
                ...
                $date = $_POST['product'][9]['value'];

                $effectiveDate = date('Y-m-d', strtotime("+3 months"));

                if(strtotime($date) > strtotime($effectiveDate))  {

                    $insert = "tbl_product(`product_name`, `variant_id`, `generic_code`, `dosage_code`, `category_code`, `product_qtyperbox`, `product_qtyperpiece`, `product_priceperpiece`,";
